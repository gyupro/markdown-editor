...
// Format time ago - now accepts translation function
const formatTimeAgo = (date: Date | null, t: ReturnType<typeof useTranslations>): string => {
  if (!date) return '';
  const seconds = Math.floor((new Date().getTime() - date.getTime()) / 1000);
  if (seconds < 5) return t('toolbar.justSaved');
  if (seconds < 60) return t('toolbar.secondsAgo', { seconds });
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) return t('toolbar.minutesAgo', { minutes });
  const hours = Math.floor(minutes / 60);
  return t('toolbar.hoursAgo', { hours });
};
...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    ...
    >
      {/* Undo/Redo buttons */}
      {onUndo && (
        <ToolbarButton onClick={onUndo} title={t('toolbar.undo')} disabled={!canUndo}>
      ...
      {onRedo && (
        <ToolbarButton onClick={onRedo} title={t('toolbar.redo')} disabled={!canRedo}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <ToolbarButton onClick={onInlineCode} title={t('toolbar.inlineCode')}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <ToolbarButton onClick={onBulletList} title={t('toolbar.bulletList')}>
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          <path d="M4 10.5c-.83 0-1.5.67-1.5 1.5s.67 1.5 1.5 1.5 1.5-.67 1.5-1.5-.67-1.5-1.5-1.5zm0-6c-.83 0-1.5.67-1.5 1.5S3.17 7.5 4 7.5 5.5 6.83 5.5 6 4.83 4.5 4 4.5zm0 12c-.83 0-1.5.68-1.5 1.5s.68 1.5 1.5 1.5 1.5-.68 1.5-1.5-.67-1.5-1.5-1.5zM7 19h14v-2H7v2zm0-6h14v-2H7v2zm0-8v2h14V5H7z"/>
        </svg>
      </ToolbarButton>
      <ToolbarButton onClick={onNumberedList} title={t('toolbar.numberedList')}>
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <ToolbarButton onClick={onCodeBlock} title={t('toolbar.codeBlock')}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <ToolbarButton onClick={onAI} title={t('toolbar.aiGenerate')}>
        <span className="font-bold text-sm">AI</span>
      </ToolbarButton>

      <Divider />

      <ToolbarButton onClick={onSelectAll} title={t('toolbar.selectAll')}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
        </svg>
      </ToolbarButton>
      <ToolbarButton onClick={onCopy} title={t('toolbar.copy')}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
        </svg>
      </ToolbarButton>
      <ToolbarButton onClick={onExportPDF} title={t('toolbar.exportPDF')}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
